...
from shorts_bot.db.repository import update_job
...
def process_video(self, source_type: str, source: str, min_duration: int, max_duration: int, chat_id: int) -> dict[str, list[str]]:
    ...
    try:
        ...
        segments = find_segments(words, min_duration, max_duration, settings.llm_api_key, settings.llm_base_url, settings.llm_model)

        outputs: list[str] = []
        _set_status(self, "RENDERING")
        for index, segment in enumerate(segments, start=1):
            output = render_short(video, words, segment, job_dir / f"short_{index}.mp4")
